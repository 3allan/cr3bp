...
:Info: **Developing a General Solution:** Write down the **solution** to the 2-body problem in terms of `r` and `\theta` using the previously developed properties of the system.
...
.. figure:: ../../images/kepler_trajectories_timed_in_e_v4.svg
    ...
    Orbital position in equal timesteps.
    The timesteps were chosen such that points fall on the `x` axis and `(0,1)`.
...
.. code-block:: matlab
    :caption: My caption. Note: The name defines an implicit target that can be referenced by using ref. Both caption and name need defined (example: :numref:`this-py`)
    :name: this-m
    :linenos:
    :lineno-start: 1
    :emphasize-lines: 1,3
...
It's possible to include files directly (use :lines: directive 1,3,5-10,20- to include lines 1, 3, 5 to 10, and 20 to the end).
